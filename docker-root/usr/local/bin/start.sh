...
do
    ...
    if [ -d /sys/class/net/tun0 ]; then
        # sed -i '/external: tun0/d' /run/danted.conf
        chmod a+w /tmp
        open_port 1080
...
# check danted for socks5 server is running
# (
#     while true; do
#         sleep 10
#         pgrep -x danted > /dev/null 2>&1
#         if ! [ "$?" = "0" ]; then
#             echo "socks5 server danted failed to start, fallback to default config"

#             # sed -i '/external: tun0/d' /etc/danted.conf
#             /usr/sbin/danted
#         fi
#     done
# )&
